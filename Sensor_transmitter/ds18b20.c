 ...
 uint8_t resetDS18B20(void)
{
   ...
   //disable int
    /* reset pulse */
	 fake_OC_setOneWire(LOW);
   __delay_cycles(DS1820_RST_PULSE);
   fake_OC_setOneWire(HIGH);

   /* wait until pullup pull 1-wire bus to high */
   __delay_cycles(DS1820_PRESENCE_WAIT);

   /* get presence pulse */
   bPresPulse = getOneWire;

   __delay_cycles(DS1820_RST_PULSE); //4244
 ...
 uint8_t readDS18B20(void)
{
   ...
   //int    
	   fake_OC_setOneWire(LOW);
   __delay_cycles(DS1820_MSTR_BITSTART);
   fake_OC_setOneWire(HIGH);
   __delay_cycles(DS1820_BITREAD_DLY);
   //int
   value|=(getOneWire<<i);
   
      __delay_cycles(120*32);
   }
   return(value);
 ...
 * RETURN:     -
 ******************************************************************************/
 void writeDS18B20(uint8_t data)
{
   uint8_t i;
   for (i=0; i < 8; i++)      /* writes byte, one bit at a time */
   {
     //int dis
	   fake_OC_setOneWire(LOW);
     __delay_cycles(DS1820_MSTR_BITSTART);
    fake_OC_setOneWire((data&0x1)&&0x01);
     __delay_cycles(DS1820_BITWRITE_DLY);
     fake_OC_setOneWire(HIGH);
     data=data>>1;
          __delay_cycles(DS1820_BITREAD_DLY);
...
setOneWireDir(0);  //input

setOneWire(0);  //keep always low
}
 ...
 void readTemp(DS_sensor_t *sensor)
{
    ...
    setOneWireDir(HIGH) ;
     //wait hier for 750ms
    __delay_cycles(DS1820_DELAY_750MS);
    setOneWireDir(LOW);
#ifdef PARASITE_POWER
    setOneWire(LOW);
#endif
   ...
   for(i=0;i<8;i++)
    ...
    sensor->temp=  (sensor->temp&0xFFF0)  | (((sensor->temp&0x0F)*625)/1000);
    for(i=0;i<6;i++)
    readDS18B20();
}
//
